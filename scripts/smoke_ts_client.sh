#!/usr/bin/env bash
# Cross-runtime smoke: start the py JSON-RPC server, drive it with the real
...
cd "$REPO_DIR"
# Server output to /dev/null so a surviving child can never hold this
# script's stdout open; `uv run` spawns python as a child, so kill both.
uv run --no-sync python -m forthic.jsonrpc.server --port "$PORT" > /dev/null 2>&1 &
SERVER_PID=$!
