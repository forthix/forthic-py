#!/usr/bin/env node
/**
 * Cross-runtime smoke: drive the forthic-py JSON-RPC server with the real
...
(async () => {
  ...
  // 1. Mixed-type stack round-trips through the python runtime
  ...
  // 4. Rich errors surface as RemoteRuntimeError with python metadata
  let threw = false;
  try {
    ...
    assert(e.runtime === 'python', `error runtime: ${e.runtime}`);
    assert(e.errorType === 'UnknownWordError', `error type: ${e.errorType}`);
    assert(e.context && e.context.word_name === 'NO-SUCH-WORD', 'error context intact');
  }
  assert(threw, 'unknown word raised');

  console.log('cross-runtime smoke OK (ts client <-> py server)');
})().catch((e) => {
